...
import Datatable from "./joinrequest/Datatable";
import { adminRoot } from "constants/defaultValues";
import download from "downloadjs";
const Joinrequest = ({ match, history }) => {
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.PARTNERS + "/exportcsv", {
  ...
  const fetchData = async () => {
    ...
    senddata["role"] = "diner";
    try {
      let { data } = await api.get(axiosURLS.PARTNERS, { params: senddata });
  ...
  return (
    <>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.join_requests" match={match} />
