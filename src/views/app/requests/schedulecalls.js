...
import Datatable from "./schedulecalls/Datatable";
import { adminRoot } from "constants/defaultValues";
import download from "downloadjs";
const Schedulecalls = ({ match, history }) => {
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.SCHEDULE_CALL + "/exportcsv", {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.SCHEDULE_CALL, { params: senddata });
  ...
  return (
    <>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.schedule_call" match={match} />
