...
import Datatable from "./patrontypes/Datatable";
import Addmodal from "./patrontypes/Addmodal";
import Deletealert from "../elements/Deletealert";
import download from "downloadjs";
const PatronTypes = ({ match }) => {
      ...
      if (res) {
        await Promise.all(
          selectedItems.map(async (item) => {
            await api.delete(axiosURLS.PATRON_TYPE + "/" + item);
          })
        );
        setSelectedItems([]);
        fetchData();
        NotificationManager.success("Partner Type Deleted successfully", "Success", 3000, null, null, "");
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.PATRON_TYPE, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.PATRON_TYPE + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.patron_type" match={match} />
