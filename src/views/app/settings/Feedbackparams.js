...
import Datatable from "./feedbackparams/Datatable";
import Addmodal from "./feedbackparams/Addmodal";
import Deletealert from "../elements/Deletealert";
import download from "downloadjs";
const Feedbackparams = ({ match }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedPageSize, setSelectedPageSize] = useState(4);
  const [selectedOrderOption, setSelectedOrderOption] = useState({ column: "", order: "" });
  ...
  const deleteSelected = async (res) => {
    ...
    if (res) {
      ...
      setSelectedItems([]);
      fetchData();
      NotificationManager.success("Feedback Parameter Deleted successfully", "Success", 3000, null, null, "");
    }
    setIsLoading(false);
  };
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.FEEDBACK_PRAMS, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.FEEDBACK_PRAMS + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.feedback_params" match={match} />
