...
import Datatable from "./cuisines/Datatable";
import Addmodal from "./cuisines/Addmodal";
import Deletealert from "../elements/Deletealert";
import download from "downloadjs";
const Cuisines = ({ match }) => {
  ...
  const deleteSelected = async (res) => {
    ...
    try {
      if (res) {
        ...
        setSelectedItems([]);
        fetchData();
        NotificationManager.success("Cuisine Deleted successfully", "Success", 3000, null, null, "");
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.CUISINES, {
        params: senddata,
      });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.CUISINES + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.cuisine" match={match} />
