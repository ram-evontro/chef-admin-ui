...
import Datatable from "./mealtypes/Datatable";
import Addmodal from "./mealtypes/Addmodal";
import Deletealert from "../elements/Deletealert";
import download from "downloadjs";
const Mealtypes = ({ match }) => {
      ...
      if (res) {
        await Promise.all(
          selectedItems.map(async (item) => {
            await api.delete(axiosURLS.MEAL_TYPES + "/" + item);
          })
        );
        setSelectedItems([]);
        fetchData();
        NotificationManager.success("Meal Type Deleted successfully", "Success", 3000, null, null, "");
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.MEAL_TYPES, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.MEAL_TYPES + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.meal_types" match={match} />
