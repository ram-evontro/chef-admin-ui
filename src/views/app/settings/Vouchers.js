...
const Vouchers = ({ match }) => {
  ...
  const deleteSelected = async (res) => {
    ...
    try {
      if (res) {
        ...
        setSelectedItems([]);
        fetchData();
        NotificationManager.success("Voucher Deleted successfully", "Success", 3000, null, null, "");
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.VOUCHER, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.VOUCHER + "/exportcsv", {
  ...
  const setStatusSelected = async (status) => {
    ...
    setSelectedItems([]);
    fetchData();
     NotificationManager.success("Voucher status changed successfully", "Success", 3000, null, null, "");
    setIsLoading(false);
  };
