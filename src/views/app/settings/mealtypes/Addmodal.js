...
const Addmodal = ({ modalOpen, toggleModal, fetchData, editformdata, modalFor }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      if (modalFor === "edit") {
        await api.patch(axiosURLS.MEAL_TYPES + "/" + id, formdata);
        NotificationManager.success("Meal Type Edited successfully", "Success", 3000, null, null, "");
      } else {
        await api.post(axiosURLS.MEAL_TYPES, formdata);
        NotificationManager.success("Meal Type Added successfully", "Success", 3000, null, null, "");
      }
      fetchData();
      toggleModal();
    } catch (err) {
