...
const Addmodal = ({ modalOpen, toggleModal, fetchData, editformdata, modalFor,countries }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      if (modalFor === "edit") {
        await api.patch(axiosURLS.CITY + "/" + id, formdata);
        NotificationManager.success("City Edited successfully", "Success", 3000, null, null, "");
      } else {
        await api.post(axiosURLS.CITY, formdata);
        NotificationManager.success("City Added successfully", "Success", 3000, null, null, "");
      }
      fetchData();
      toggleModal();
    } catch (err) {
    ...
    <Modal isOpen={modalOpen} toggle={toggleModal} wrapClassName="modal-right" backdrop="static">
      ...
      <ModalBody>
        ...
        <FormGroup>
          ...
          <select name="country" value={formdata.country ? formdata.country : ""} onChange={handleChange} id="country" className="form-control">
            <option value="" key="blank">
              Select Country
            </option>
            {countries && countries.map((data, index) => {
              return (
                <option value={data.id} key={`sli_${index}`}>
                  {data.name}
                </option>
              );
            })}
