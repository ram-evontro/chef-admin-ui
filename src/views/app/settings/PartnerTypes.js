...
import Datatable from "./partnertypes/Datatable";
import Addmodal from "./partnertypes/Addmodal";
import Deletealert from "../elements/Deletealert";
import download from "downloadjs";
const PartnerTypes = ({ match }) => {
      ...
      if (res) {
        await Promise.all(
          selectedItems.map(async (item) => {
            await api.delete(axiosURLS.PARTNER_TYPE + "/" + item);
          })
        );
        setSelectedItems([]);
        fetchData();
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.PARTNER_TYPE, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.PARTNER_TYPE + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.partner_type" match={match} />
