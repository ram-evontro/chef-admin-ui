...
import Datatable from "./addons/Datatable";
import Addmodal from "./addons/Addmodal";
...
const Addons = ({ match }) => {
      ...
      if (res) {
        await Promise.all(
          selectedItems.map(async (item) => {
            await api.delete(axiosURLS.ADDONS_CATEGORIES + "/" + item);
          })
        );
        setSelectedItems([]);
        fetchData();
        NotificationManager.success("Addon Deleted successfully", "Success", 3000, null, null, "");
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    ...
    try {
      let { data } = await api.get(axiosURLS.ADDONS_CATEGORIES, { params: senddata });
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.ADDONS_CATEGORIES + "/exportcsv", {
  ...
  return (
    <React.Fragment>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.addon" match={match} />
