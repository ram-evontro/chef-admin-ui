...
const Addmodal = ({ modalOpen, toggleModal, fetchData, editformdata, modalFor }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      if (modalFor === "edit") {
        await api.patch(axiosURLS.CUISINES + "/" + id, formdata);
        NotificationManager.success("Cuisine Edited successfully", "Success", 3000, null, null, "");
      } else {
        await api.post(axiosURLS.CUISINES, formdata);
        NotificationManager.success("Cuisine Added successfully", "Success", 3000, null, null, "");
      }
      fetchData();
      toggleModal();
    } catch (err) {
