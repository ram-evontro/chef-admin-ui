...
const BestSellers = ({ users }) => {
  const [data, setData] = useState([]);
  useEffect(() => {
    if (users) {
      let tempdata = [];
      Object.keys(users).map((user) => {
        let row = { name: users[user].details.name, mobile: users[user].details.mobile, email: users[user].details.email, total_orders: users[user].count };
        tempdata.push(row);
      });
      setData(tempdata);
    }
