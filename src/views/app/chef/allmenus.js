...
import Datatable from "./allmenus/Datatable";
...
const Allmenus = ({ match, history }) => {
  ...
  const setStatusSelected = async (status) => {
    ...
    setSelectedItems([]);
    fetchData();
    NotificationManager.success("Menu status changed successfully", "Changed", 3000, null, null, "");
    setIsLoading(false);
  };
  const toggleStatusSingle = async (id, setto) => {
    ...
    try {
      await api.patch(axiosURLS.MENU + "/" + id, formdata);
      fetchData();
      NotificationManager.success("Menu status changed successfully", "Changed", 3000, null, null, "");
    } catch (err) {
      console.log(err);
  ...
  const updateAction = async (action, id) => {
    ...
    if (action === "view") {
      history.push(`${adminRoot}/chef/menuview/?menu=` + id);
    }
  };
  useEffect(() => {
    setCurrentPage(1);
  ...
  return (
    <>
      ...
      <Menuadd fetchData={fetchData} id={false} chefs={chefs} chefTypes={chefTypes} modalOpen={modalOpen} toggleModal={() => setModalOpen(!modalOpen)} />
