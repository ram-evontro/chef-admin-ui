...
import Addmodal from "./events/Addmodal";
...
const Events = ({ match, history }) => {
      ...
      try {
        await Promise.all(
          selectedItems.map(async (item) => {
            await api.delete(axiosURLS.EVENT + "/" + item);
          })
        );
        setSelectedItems([]);
        fetchData();
  ...
  const exportCSV = async () => {
    setIsLoading(true);
    try {
      let response = await api.post(axiosURLS.EVENT + "/exportcsv", {
  ...
  const setStatusSelected = async (status) => {
    ...
    await Promise.all(
      selectedItems.map(async (item) => {
        await api.patch(axiosURLS.EVENT + "/" + item, formdata);
      })
    );
    setSelectedItems([]);
    fetchData();
  ...
  const toggleStatusSingle = async (id, setto) => {
    ...
    try {
      await api.patch(axiosURLS.EVENT + "/" + id, formdata);
      fetchData();
      NotificationManager.success("Menu status changed successfully", "Changed", 3000, null, null, "");
    } catch (err) {
      console.log(err);
